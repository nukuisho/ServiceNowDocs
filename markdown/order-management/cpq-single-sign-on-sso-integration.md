...
description: If you intend to use ServiceNow CPQ for a headless use case \(such as exposing ServiceNow CPQ on a website or eCommerce platform\), please fill out and submit an SSO Setup Request Form that details the identity provider \(IdP\) you are using for single sign-on \(SSO\).
locale: en-US
...
To set up an application that supports the Authorization code flow for ServiceNow CPQ, start by following the Okta Developer instructions: [Implement authorization by grant type: set up your app](https://developer.okta.com/docs/guides/implement-grant-type/authcode/main/#set-up-your-app)
