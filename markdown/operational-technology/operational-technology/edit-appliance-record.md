...
-   **Management Interface**: Connects the appliance to an out-of-band network for communication with the .
    -   **Management Endpoint**: Sets the IP address for the Sensor which it uses to communicate with the .
    -   Management Subnet: Sets the subnet mask/Classless Inter-Domain Routing \(CIDR\) for the appliance communication with the .
...
The **Services Config** tab includes the **Log Streaming Service**, and the **Web Management Service**. You can use the Log Streaming Service to stream logs from appliances to the . This service also retains logged information for a longer period, which can assist in quicker customer issue resolution.
